      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 mt-1">Manage employee payroll and view history.</p>
          </div>
        ...
        {activeTab === "pay" ? (
           <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
             {employees.map((emp) => (
               <div key={emp.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-lg transition-shadow duration-300 flex flex-col items-center text-center group">
                 <div className="w-20 h-20 rounded-full bg-blue-50 mb-4 flex items-center justify-center text-2xl font-bold text-blue-600 overflow-hidden border-2 border-white shadow-inner">
                    {emp.photoUrl ? <img src={emp.photoUrl} alt={emp.name} className="w-full h-full object-cover"/> : emp.name.charAt(0)}
                 </div>
                 <h3 className="text-lg font-bold text-gray-800 group-hover:text-blue-600 transition-colors">{emp.name}</h3>
                 <p className="text-sm text-gray-500 mb-4">{emp.role}</p>
                 
                 <div className="w-full bg-gray-50 rounded-lg p-3 mb-4">
                   <p className="text-xs text-gray-400 uppercase tracking-wide font-semibold">Base Salary</p>
                   <p className="text-xl font-bold text-gray-800">₹{emp.salary || "N/A"}</p>
                 </div>

                 <button
                   onClick={() => handlePayClick(emp)}
                   className="w-full py-2.5 bg-gray-900 text-white rounded-xl font-medium hover:bg-black transition-colors shadow-lg shadow-gray-200 active:scale-95"
                 >
                   Pay Now
                 </button>
               </div>
                  ...
                  {payments.length === 0 ? (
                    ...
                    payments.map((p) => (
                      <tr key={p.id} className="hover:bg-blue-50/50 transition-colors group">
                        <td className="px-6 py-4 font-medium text-gray-800">{p.employeeName}</td>
                        <td className="px-6 py-4 text-gray-600">
                          <span className="bg-blue-100 text-blue-700 px-2 py-1 rounded text-xs font-semibold">{p.salaryMonth}</span>
                        </td>
                        <td className="px-6 py-4 text-gray-500 text-sm">{format(new Date(p.paymentDate), "MMM dd, yyyy")}</td>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
              <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                <h3 className="font-bold text-lg text-gray-800">
                  {editingPaymentId ? "Edit Payment" : "Pay Salary"} - <span className="text-blue-600">{selectedEmployee.name}</span>
                </h3>
                <button onClick={resetForm} className="text-gray-400 hover:text-red-500 transition-colors">✕</button>
              </div>
              ...
              <div className="px-6 py-4 bg-gray-50 flex justify-end gap-3">
                 ...
                 <button 
                  ...
                  disabled={loading}
                  className="px-6 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 active:bg-blue-800 transition-colors shadow-lg shadow-blue-200 disabled:opacity-50"
                 >
                   {loading ? "Processing..." : (editingPaymentId ? "Update Payment" : "Confirm Payment")}
                 </button>
              </div>
            </div>
          </div>
