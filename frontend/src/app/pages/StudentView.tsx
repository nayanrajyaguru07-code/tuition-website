      ...
      {fees && (
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-gray-900">Fee Overview</h3>
            {(() => {
              const due = fees.dueFee || 0;
              const total = fees.totalFee || 0;
              let status = "Paid";
              ...
              return (
                <span
                  className={`px-3 py-1 rounded-full text-sm font-semibold border ${color}`}
                >
                  {status}
                </span>
              );
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="bg-red-50 p-4 rounded-xl border border-red-100 text-center">
              <div className="text-xs uppercase text-red-600 font-bold mb-1">
                Due
              </div>
              <div className="text-xl font-extrabold text-red-800">
                ₹{fees.dueFee}
              </div>
            </div>
          </div>
            ...
            {history.map((h, i) => (
              ...
              >
                ...
                <div className="text-right">
                  <div className="text-xs font-semibold bg-gray-200 px-2 py-0.5 rounded text-gray-700">
                    {h.paymentMethod}
                  </div>
                  {h.remarks && (
      ...
      <div className="mt-6 grid grid-cols-2 gap-4">
        {student.idProofUrl && (
          <a
            ...
            target="_blank"
            className="block text-center py-2 bg-orange-100 text-orange-700 rounded-lg hover:bg-orange-200 text-sm font-semibold transition"
          >
            View ID Proof
          </a>
        )}
        {student.admissionProofUrl && (
          <a
            href={student.admissionProofUrl}
            target="_blank"
            className="block text-center py-2 bg-orange-100 text-orange-700 rounded-lg hover:bg-orange-200 text-sm font-semibold transition"
          >
            Admission Proof
