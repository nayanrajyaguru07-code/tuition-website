...
export default function StaffView({ id }: { id: string }) {
  ...
  if (!staff) return <div className="p-10 text-center text-red-500">Staff not found</div>;
  ...
  return (
    <div className="max-h-[80vh] overflow-y-auto pr-2 custom-scrollbar">
       ...
       <div className="flex items-center gap-6 mb-8 bg-orange-50 p-6 rounded-2xl border border-orange-100">
          ...
          />
          <div>
            <h2 className="text-2xl font-extrabold text-gray-900">{staff.name}</h2>
            <div className="flex items-center gap-2 mt-1">
               <span className="bg-orange-100 text-orange-700 text-xs font-bold px-2 py-0.5 rounded-full uppercase">{staff.role}</span>
            </div>
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
          ...
          <DetailRow label="Joining Date" value={staff.dateOfJoining ? new Date(staff.dateOfJoining).toLocaleDateString() : "-"} />
       </div>
