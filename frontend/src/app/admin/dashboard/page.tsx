      ...
      `}>
        ...
        <div className="p-8 pb-4 hidden md:block">
           <div className="flex items-center gap-3 mb-1">
             ...
             <div>
                ...
                <h1 className="text-xl font-bold tracking-tight text-gray-900 leading-none">AdminPortal</h1>
                <span className="text-[10px] font-semibold tracking-wider text-gray-400 uppercase">Management</span>
        ...
        */}
        <div className="md:hidden p-6 flex justify-between items-center bg-gray-50/50">
             ...
             <button onClick={() => setMobileMenuOpen(false)} className="bg-white p-2 rounded-full shadow-sm text-gray-500">
               <X size={18} />
             </button>
        </div>
        ...
        <div className="px-4 py-2 space-y-1">
           <p className="px-4 text-xs font-bold text-gray-400 uppercase tracking-widest mb-2 mt-4">Dashboards</p>
           {["overview", "directory"].map((tab) => (
              ...
              >
                 {tab === "overview" ? <TrendingUp size={18} /> : <Users size={18} />}
        ...
        <div className="px-4 py-2 mt-4">
           <p className="px-4 text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Context</p>
        ...
        <div className="mt-auto p-4 space-y-3">
           <button
              onClick={() => { setIsAddHostelDialogOpen(true); setMobileMenuOpen(false); }}
           ...
           <button
              onClick={() => { handleOpenFeeDialog(); setMobileMenuOpen(false); }}
           ...
           <div className="pt-4 border-t border-gray-100 mt-4">
              <Link href="/home" className="flex items-center gap-3 px-4 py-3 text-sm font-medium text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors">
                  <LogOut size={18} />
                  <span>Logout</span>
              </Link>
           </div>
        ...
        <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
           <div>
              <h2 className="text-2xl font-bold text-gray-900">
                {activeTab === 'overview' ? 'Dashboard Overview' : 'Directory Management'}
              </h2>
            ...
            {activeTab === "overview" && (
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                  ...
                  <StatCard title="Revenue (Mo)" value={`₹${recoveryData?.collected ?? 0}`} icon={<Wallet />} />
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                    ...
                    <div className="h-80 w-full">
                      <ResponsiveContainer>
                        <BarChart data={financeData} barGap={8}>
                          ...
                          />
                          <Bar dataKey="income" fill="#F97316" radius={[6,6,6,6]} barSize={32} />
                          <Bar dataKey="expense" fill="#Fee2e2" radius={[6,6,6,6]} barSize={32} />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
                  ...
                  <div className="bg-white rounded-3xl p-8 shadow-[0_2px_20px_rgba(0,0,0,0.02)] border border-gray-100/50">
                    <h2 className="text-lg font-bold text-gray-900 mb-6">Monthly Recovery</h2>
                    <div className="h-64 relative">
                       <ResponsiveContainer>
                         <PieChart>
                           ...
                           >
                             {pieData.map((_, i) => (
                               <Cell key={i} fill={i === 0 ? "#F97316" : "#f3f4f6"} stroke="none" />
                             ))}
                           </Pie>
                           <Tooltip />
                         </PieChart>
                       ...
                       <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                          <span className="text-3xl font-bold text-gray-900">
                             {pieData[0]?.value ? Math.round((pieData[0].value / (pieData[0].value + pieData[1].value)) * 100) : 0}%
                          </span>
                    ...
                    <div className="mt-6 space-y-3">
                       <div className="flex items-center justify-between text-sm">
                          ...
                          <span className="font-semibold text-gray-900">₹{recoveryData?.collected ?? 0}</span>
                       </div>
                       <div className="flex items-center justify-between text-sm">
                          <div className="flex items-center gap-2 text-gray-500">
                          ...
                          <span className="font-semibold text-gray-900">₹{recoveryData?.pending ?? 0}</span>
                       </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(0,0,0,0.02)] border border-gray-100/50 overflow-hidden">
                    ...
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50/50 text-gray-400 uppercase tracking-wider text-xs">
                        <tr>
                          <th className="px-8 py-5 text-left font-semibold">Hostel Name</th>
                          <th className="px-8 py-5 text-right font-semibold">Revenue</th>
                          <th className="px-8 py-5 text-right font-semibold">Expense</th>
                          <th className="px-8 py-5 text-right font-semibold">Net Profit</th>
                        ...
                        {performanceData.map((h) => {
                          ...
                          return (
                            <tr key={h.id} className="hover:bg-gray-50/80 transition-colors">
                              <td className="px-8 py-5 font-medium text-gray-900">{h.name}</td>
                              ...
                              <td className={`px-8 py-5 text-right font-bold ${profit >= 0 ? 'text-green-600' : 'text-red-500'}`}>
                                {profit >= 0 ? '+' : ''}₹{profit.toLocaleString()}
                              </td>
                              <td className="px-8 py-5 text-center">
                                 <span className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold ${profit > 0 ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
            ...
            {activeTab === "directory" && (
              <div className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(0,0,0,0.02)] border border-gray-100/50 overflow-hidden min-h-[600px]">
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50/50 text-gray-400 uppercase tracking-wider text-xs">
                    <tr>
                      <th className="px-8 py-5 text-left font-semibold">Profile</th>
                      <th className="px-8 py-5 text-left font-semibold">Contact Info</th>
                      <th className="px-8 py-5 text-left font-semibold">
                        {directoryTab === "students" ? "Hostel Assigned" : "Role / Designation"}
                      </th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filteredList.map((i) => (
                      <tr key={i.id} className="group hover:bg-orange-50/30 transition-colors">
                        <td className="px-8 py-5">
                           <div className="flex items-center gap-3">
                              ...
                              <span className="font-semibold text-gray-900">{i.fullName || i.name}</span>
                           </div>
                        </td>
                        <td className="px-8 py-5">
                          <div className="flex flex-col gap-0.5">
                             <span className="text-gray-900 font-medium">{i.email}</span>
                             <span className="text-gray-500 text-xs">{i.studentMobileNo || i.phone || "No phone"}</span>
                          </div>
                        </td>
                        <td className="px-8 py-5">
                           {i.hostel?.hostelName ? (
      ...
      <Dialog open={isAddHostelDialogOpen} onOpenChange={setIsAddHostelDialogOpen}>
        <DialogContent>
           ...
           <div className="space-y-4 py-4">
              ...
              >
                {loading ? <div className="h-5 w-5 animate-spin rounded-full border-2 border-white border-t-transparent mx-auto" /> : "Create Hostel"}
              </button>
           </div>
        </DialogContent>
      </Dialog>
